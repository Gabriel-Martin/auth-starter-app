...
module.exports = db => {
  ...
  // db.createModel takes in 3 paramters (1 is optional)
  ...
  // modelOptions - alternate settings to change the behaviour of how our data is
  // validated, saved or queried.
  ...
  User.define("comparePassword", function(password) {
    return bcrypt
      ...
      .catch(
        bcrypt.MISMATCH_ERROR, // catch any password errors, and return custom error message
  ...
  User.pre("save", function(next) {
    User.filter({ email: this.email }).then(users => {
      ...
      // if longer than zero, proceed with an error
      if (users.length > 0) {
